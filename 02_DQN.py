...
def get_model(inputs_shape):
    ...
    注意: tensorlayer 和 tensorflow 同样的层、同样功能的参数，名字居然不一样！还有 keras 里面的 layer, 各种坑
         但我们后面的代码都用上了 tensorlayer
    """
    # jy: inputs_shape 即输入矩阵的形状
    ni = tl.layers.Input(inputs_shape, name='observation')
    # jy: 和之前 Fasion MNIST 一样，我们用 Dense 作为输出; 这里没有加一个 Dense 层, 主要是因为这里的状态
    #     空间只有 16 个, 这样的模型拟合能力已经足够
    #     W_init 和 b_init : 用正态分布 (均值 0, 方差 0.01) 的方式随机初始化参数
...
# jy: 迭代次数
num_episodes = 2000
# display the game environment (是否渲染游戏)
render = False  
    ...
    if args.train:
        ...
        for i in range(num_episodes):
            ...
            for j in range(99):  
                ...
                ## Get new state and reward from environment (输入到环境, 获得下一步的 state, reward, done)
                s1, r, d, _ = env.step(a[0])
                if render: env.render()
                ## Obtain the Q' values by feeding the new state through our network
                #  把 new-state 放入, 预测下一个 state 的所有动作的 Q 值
                ...
                # jy: 获取下一个状态中最大 Q 值
                maxQ1 = np.max(Q1)  
                # jy: 用 allQ (现在状态的 Q 值) 构建更新的 target; 因为只有被选择那个动作才会被更新到
                targetQ = allQ
            ...
            # all_episode_reward 用于记载每一次更新的总和, 为了能够更加看清变化, 所以大部分是前面的, 只有一部分是后面的
            if i == 0:
