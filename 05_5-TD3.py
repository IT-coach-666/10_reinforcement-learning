...
class QNetwork(Model):
    ...
    def __init__(self, num_inputs, num_actions, hidden_dim, init_w=3e-3):
        ...
        self.linear1 = Dense(n_units=hidden_dim, act=tf.nn.relu,
                             W_init=w_init, in_channels=input_dim,
                             name='q1')
        self.linear2 = Dense(n_units=hidden_dim, act=tf.nn.relu,
                             W_init=w_init, in_channels=hidden_dim,
                             name='q2')
        # jy: 网络输出层(输出一个值)
...
class PolicyNetwork(Model):
    """ 
    the network for generating non-determinstic (Gaussian distributed)
    action from the state input 
    """
    def __init__(self, num_inputs, num_actions, hidden_dim,
        ...
        self.linear1 = Dense(n_units=hidden_dim, act=tf.nn.relu,
                             W_init=w_init, in_channels=num_inputs,
                             name='policy1')
        self.linear2 = Dense(n_units=hidden_dim, act=tf.nn.relu,
                             W_init=w_init, in_channels=hidden_dim,
                             name='policy2')
        ...
        self.output_linear = Dense(
            n_units=num_actions, W_init=w_init, 
    ...
    def evaluate(self, state, eval_noise_scale):
        ...
        # jy: 通过 state 计算 action (action 范围是 [-1, 1])
        # jy-bug-1 相关: 尽量不要调用 self.forward(state), 直接调用 self(state)
        #action = self.forward(state)           
        action = self(state)
        # jy: 映射到游戏的 action 取值范围
        action = self.action_range * action

        # jy: add noise (建立一个正态分布)
        normal = tfp.distributions.Normal(0, 1)
        # jy: 对噪声进行上下限裁剪; eval_noise_scale
        eval_noise_clip = 2 * eval_noise_scale
        # jy: 弄个一个 noisy 和 action 的 shape 一致, 然后乘以 scale
        noise = normal.sample(action.shape) * eval_noise_scale
        # jy: 对 noisy 进行剪切, 不要太大也不要太小
    ...
    def get_action(self, state, explore_noise_scale):
        ...
        输出 action, 维度如: [64, 1]
        """
        # jy: forward 函数接受输入 state, 输出 action (激活函数为 tanh, 所以范围是 [-1, 1])
        # jy-bug-1 相关: 尽量不要调用 self.forward(state), 直接调用 self(state)
        #action = self.forward([state])  
        action = self([state])
        # jy: 将 action 变成矩阵
        ...
        # add noise 
        # jy: 生成 normal 这样一个正态分布
        normal = tfp.distributions.Normal(0, 1)
        # jy: 在正态分布中抽样一个和 action 一样 shape 的数据, 然后乘以 scale
        noise = normal.sample(action.shape) * explore_noise_scale 
        # jy: action 乘以动作的范围, 加上 noise
...
class TD3_Trainer():
    ...
    def __init__(self, replay_buffer, hidden_dim, action_range,
                 state_dim, action_dim, policy_target_update_interval=1,
    ...
    def update(self, batch_size, eval_noise_scale, reward_scale=10.,
               gamma=0.9, soft_tau=1e-2):
        """
        update all networks in TD3 
        """
        # jy: 计算更新次数
        self.update_cnt += 1
        # jy: 从 buffer sample 数据
        state, action, reward, next_state, done = self.replay_buffer.sample(batch_size)
        # jy: expand dim, 调整形状, 方便输入网络
        reward = reward[:, np.newaxis]

        # jy: 输入 s', 从 target_policy_net 计算 a' (注意: 此处有加 noisy)
        ...
        # jy: 归一化 reward (有正有负); normalize with batch mean and std;
        ...
        # jy: Training Q Function; 把 next_state 和 new_action 堆叠在一起
        #     输入到 target_q_net, 从两个 qnet 中取最小值
        ...
        # jy: 基于 target_q_min 计算 target_q, 用于作为基准参照更新 q_net 网络
        #     把 done 从布尔变量改为 int (if done==1, only reward)
        done = done[:, np.newaxis]
        target_q_value = reward + (1 - done) * gamma * target_q_min  
        # jy: 基于当前的 state 和 action 构造 q_net 网络的输入
        q_input = tf.concat([state, action], 1)  
        # jy: 更新 q_net1 网络 (此处和 DQN 一样)
        ...
        # jy: Training Policy Function; policy 不经常 update (qnet 更新
        #     一定次数才 update 一次)
        if self.update_cnt % self.policy_target_update_interval == 0:
            # jy: 更新 policy_net
            with tf.GradientTape() as p_tape:
                # jy: 计算 action = Policy(s) (注意: 此处没有 noise)
                ...
                # jy: 对一个 batch_size 的 Q 值取平均, 并取负值 (即梯度上升)
            ...
            # jy: Soft update the target nets; 软更新 target_network 三个;
            #     将 q_net 网络参数软更新(qnet 网络参数和 target_q_net 网络参
            #     数各占一定比例) 至 target_q_net
            ...
            # jy: 将 policy_net 网络参数软更新至 target_policy_net
    ...
    def save_weights(self):  
        ...
        tl.files.save_npz(self.q_net1.trainable_weights,
                          name='model_q_net1.npz')
        tl.files.save_npz(self.q_net2.trainable_weights,
                          name='model_q_net2.npz')
        tl.files.save_npz(self.target_q_net1.trainable_weights,
                          name='model_target_q_net1.npz')
        tl.files.save_npz(self.target_q_net2.trainable_weights,
                          name='model_target_q_net2.npz')
        tl.files.save_npz(self.policy_net.trainable_weights,
                          name='model_policy_net.npz')
        tl.files.save_npz(self.target_policy_net.trainable_weights,
                          name='model_target_policy_net.npz')

    def load_weights(self):  
        """
        load trained weights
        """
        tl.files.load_and_assign_npz(name='model_q_net1.npz',
                                     network=self.q_net1)
        tl.files.load_and_assign_npz(name='model_q_net2.npz',
                                     network=self.q_net2)
        tl.files.load_and_assign_npz(name='model_target_q_net1.npz',
                                     network=self.target_q_net1)
        tl.files.load_and_assign_npz(name='model_target_q_net2.npz',
                                     network=self.target_q_net2)
        tl.files.load_and_assign_npz(name='model_policy_net.npz',
                                     network=self.policy_net)
        tl.files.load_and_assign_npz(name='model_target_policy_net.npz',
                                     network=self.target_policy_net)
...
def run(mode):
    ...
    td3_trainer=TD3_Trainer(
        replay_buffer, hidden_dim, action_range, state_dim, action_dim,
    ...
    if mode == "train":
        # jy: 记录步数
        frame_idx = 0
        # jy: 记录每个 EP 的总 reward
        rewards = []       
        t0 = time.time()
        # jy: 如果小于最大步数, 就继续训练
        while frame_idx < max_frames:
            # jy: 初始化 state, 并整理 state 的类型
            state = env.reset()  
            state = state.astype(np.float32)
            episode_reward = 0
            # jy: 第一次需要进行初始化 trainer
            if frame_idx < 1:      
                print('intialize')
            ...
            for step in range(max_steps):
                # jy: 如果小于 500 步, 就随机; 如果大于就用 get_action 方法获取 action;
                if frame_idx > explore_steps: 
                    # jy: 带有 noisy 的 action
                    action = td3_trainer.policy_net.get_action(
                        state, explore_noise_scale=1.0)  
                else:
                ...
                # jy: 记录数据在 replay_buffer
                replay_buffer.push(state, action, reward, next_state, done)

                # jy: 赋值 state, 累计总 reward 和步数
                state = next_state
                episode_reward += reward
                frame_idx += 1

                # jy: 如果数据超过一个 batch_size 的大小, 则开始更新
                if len(replay_buffer) > batch_size:
                    # jy: 注意: 这里可以更新多次
                    for i in range(update_itr):
                        # jy: 
                        td3_trainer.update(
                            batch_size, eval_noise_scale=0.5,
                            reward_scale=1.)
